...
if uploaded_file:
    ...
    if not df.empty:
        ...
        st.sidebar.header("🔧 Maintenance Reset")
        ...
        # Estimate Shaker Unit Life Remaining (%)
        ...
        shaker_status = "🟢 OK"
        if latest_shaker_life < failure_threshold:
            shaker_status = "🔴 At Risk"
        ...
        with st.expander("📈 Screen Utilization Over Time"):
            chart1 = alt.Chart(df.reset_index()).mark_line().encode(
                x='index',
                y='Screen Utilization (%)',
                tooltip=['Screen Utilization (%)']
            ).properties(width=800, height=300)
            st.altair_chart(chart1, use_container_width=True)

        with st.expander("📈 Screen Life Remaining Trend"):
            chart2 = alt.Chart(df.reset_index()).mark_line(color='green').encode(
                x='index',
                y='Screen Life Used (%)'
            ).properties(width=800, height=300)
        ...
        with st.expander("📈 Shaker Output and Alerts"):
            chart3 = alt.Chart(df.reset_index()).mark_line(color='orange').encode(
                x='index',
                y='SHAKER Output',
                tooltip=['SHAKER Output']
            ).properties(width=800, height=300)
            st.altair_chart(chart3, use_container_width=True)

        with st.expander("📈 Shaker Unit Life Remaining"):
            chart4 = alt.Chart(df.reset_index()).mark_line(color='purple').encode(
                x='index',
                y='Shaker Life Remaining (%)'
            ).properties(width=800, height=300)
